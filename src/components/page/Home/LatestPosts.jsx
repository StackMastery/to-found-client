        ...
        <>
            <section className="flex justify-center">
                <div className="w-primary px-5 py-10 inline-flex flex-col justify-center">
                    ...
                    <div className="w-full gap-10 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 pt-10">
                        {!isLoading ? latestPost && latestPost.map((post) => (
                            <React.Fragment key={post._id}>
                                <motion.div
                                    whileHover={{ translateY: -10, zIndex: 999 }}
                                    className="col-span-1 bg-white w-full"
                                >
                                    <Link to={`../post/${post._id}`}>
                                        <motion.img
                                            initial={{ opacity: 0 }}
                                            whileInView={{ opacity: 1 }}
                                            className="w-full object-cover hover:border-sky-600/30 transition-colors h-[350px] border rounded-xl sm:h-[330px] md:h-[270px] lg:h-[280px] object-center"
                                            src={post.thumb}
                                            alt={post.title}
                                        />
                                    </Link>

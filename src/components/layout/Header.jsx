...
import ButtonMe from "../ui/button";
...
const Header = () => {
  ...
  return (
    <>
      <header className="flex justify-center border-b z-[9999999] bg-white dark:bg-bgBlack dark:border-white/10 w-full fixed">
                    ...
                    <div className="flex flex-col">
                      {DropDownData &&
                        DropDownData.map((data, index) => (
                          <Link
                            key={index}
                            className="hover:bg-neutral-100 dark:hover:bg-white/10 p-1 text-sm px-3 rounded-md flex items-center gap-2"
                            to={data.path}
                          >
                            {data.icon} {data.pathName}
                          </Link>
                        ))}
                    </div>
                  </PopoverContent>
        ...
        <div className="fixed bottom-0 w-full lg:hidden py-3 border-t z-[9999] px-5 flex justify-around bg-white dark:bg-bgBlack dark:border-white/10">
          ...
          <Popover>
            <PopoverTrigger>
              <div className="flex flex-col gap-1 items-center text-neutral-700 dark:!text-white">
                <div className="flex w-fit border dark:border-white/10 p-3 rounded-lg cursor-pointer hover:rounded-[50px] transition-all">
                  <FiPlus />
                </div>
                <span className="hidden min-[380px]:block text-sm">
                  Options
                </span>
              </div>
            </PopoverTrigger>
            <PopoverContent className="mb-5 shadow-none flex flex-col w-[220px] dark:!text-white">
              {DropDownData &&
                DropDownData.map((data, index) => (
                  <Link
                    key={index}
...
const NavIcons = ({ pathName, to, children }) => (
  <div className="flex flex-col gap-1 items-center text-neutral-700 dark:!text-white">
    <NavLink
      ...
      className={({ isActive }) =>
        `${
          isActive && `bg-sky-600 text-white rounded-[50px] hover:rounded-xl`
        } flex w-fit border dark:border-white/10 p-3 rounded-lg hover:rounded-[50px] transition-all`
      }
    >
      {children}
    </NavLink>

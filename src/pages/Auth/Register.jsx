...
import { emailRegex, lengthRegex, lowercaseRegex, uppercaseRegex } from "@/hooks/Regex";
        ...
        createUserWithEmailAndPassword(auth, email, password)
            .then((res) => {
                updateProfile(auth.currentUser, {
                    displayName: name, photoURL: `${isUploading.data}`
                }).then((res) => {
    ...
    return (
        <>
            ...
            <div className="space-y-1 ">
                ...
                <p className="text-neutral-700">Wellcome to ToFound please register to continue</p>
            </div>
            <form onSubmit={handelRegistration} className="pt-5 flex flex-col gap-3">
                ...
                <div className="border rounded-xl mt-3">
                    <input accept="image/png, image/jpeg, image/svg+xml" onChange={handelAvatarupload} type="file" id="avatar" style={{display: 'none'}} />
                    <label htmlFor="avatar" className="flex gap-3 cursor-pointer px-5 py-2 w-full justify-between items-center">
                        <div className="flex items-center gap-5">
                            {!isUploading?.data
                                ? (
                                    <img width={40} height={40} src={Avatar} alt="Avatar" className="w-10 h-10 object-cover rounded-full ring-2 ring-green-400"/>
                                )
                                : <img src={isUploading?.data} alt="Avatar"  className="w-10 h-10 object-cover rounded-full ring-2 ring-green-400"/>
                            }
                            <div className="py-1">
                                <h3 className="text-md">{isUploading?.data ? 'Avatar uploaded' : 'Upload avatar'}</h3>
                                <p className="text-sm text-neutral-600 -mt-1">
                                    {
                                        isUploading?.data ? (
                                        <span className="bg-green-600 text-green-100 px-2 rounded-full text-xs py-0">success</span>
                                        ) : isUploading === true ? (
                                        'uploading'
                                        ) : 'upload'
                                    }
                        ...
                        {
                            isUploading === true && (
                                <PiSpinnerThin size={24} className="animate-spin" color={generateRandomColor()}/>
                            )
                        }
                    </label>
                </div>
                <div className="flex gap-5 justify-between mt-2">
                    ...
                    <Button disabled={isRegisterLoading} className={'w-fit'}>
                        {isRegisterLoading ? (
                            <PiSpinnerThin size={24} className="animate-spin" color={generateRandomColor()}/>
                        ): 'Register'}
                    </Button>

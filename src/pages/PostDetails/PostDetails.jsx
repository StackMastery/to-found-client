...
=======
import { CiCalendar, CiCircleInfo, CiLocationOn, CiUser, CiWarning } from "react-icons/ci";
import { PiEnvelopeSimpleOpenThin, PiHeartBreakThin } from "react-icons/pi";
import PostDetailsLoader from "@/components/loaders/PostDetailsLoader";
...
const PostDetails = () => {
    ...
    const fetchPost = async () => {
        try {
          const { data } = await axiosSecure.get(`/post/read?postid=${id}&uid=${authInfo?.uid}`);
          return data;
        } catch (error) {
          throw new Error('Error fetching post data');
        }
                        ...
                        <div className="space-y-5">
                            <div className="flex gap-3 items-center">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs -mt-1 text-neutral-600">{moment(post?.createdAt).fromNow()}</p>
                                </div>
                            </div>
                            <hr className="bg-neutral-500"/>
                            <h1 className="text-2xl sm:text-3xl py-3 md:text-4xl font-semibold">{post?.title}</h1>
                        </div>
                        <img className="w-full md:h-[350px] lg:h-[500px] rounded-xl object-cover" src={post?.thumb} alt={post?.title} />
                        <div className="py-5">
                            <p 
                                className="htmlShowcase text-neutral-800"
                                dangerouslySetInnerHTML={{ __html: post?.des }}
                            />
                        </div>
                    </div>
                    <div className="space-y-5 w-full md:w-4/12">
                        <div className="fixed md:static w-full flex-col flex bottom-16 min-[380px]:bottom-[5.7rem] z-[9999] left-0">
                            <div className="bg-white border rounded-t-2xl md:rounded-b-2xl p-5 shadow-sm w-full">
                                <button onClick={() => setisOpneDetails(!isOpneDetails)} className="font-medium text-lg flex justify-between w-full items-center">
                                    See Details And Action
                                    <IoIosArrowDown size={25} className={`${isOpneDetails && '-rotate-180'} md:hidden`}/>
                                </button>
                                <div className={`${isOpneDetails ? '!block' : 'hidden'} md:!block`}>
                                    <div className="py-5">
                                        <hr className="bg-neutral-500"/>
                                    </div>
                                    <ul className="flex flex-wrap gap-5">
                                        <li className="flex items-center bg-neutral-100/80 hover:bg-neutral-200/90 cursor-pointer border w-fit px-3 rounded-full gap-3 capitalize text-md text-neutral-700">
                                            <CiUser size={20}/>
                                            {post?.authorName || 'N/A'}
                                        </li>
                                        <li className="flex items-center bg-neutral-100/80 hover:bg-neutral-200/90 border  cursor-pointer w-fit px-3 rounded-full gap-3 capitalize text-md text-neutral-700">
                                            <CiLocationOn size={20}/>
                                            {post?.location || 'N/A'}
                                        </li>
                                        <li className="flex items-center bg-neutral-100/80 hover:bg-neutral-200/90 border  cursor-pointer w-fit px-3 rounded-full gap-3 lowercase text-md text-neutral-700 overflow-x-hidden">
                                            <PiEnvelopeSimpleOpenThin size={20}/>
                                            <span className={`${!post?.email && '!capitalize'}`}>{post?.email|| 'N/A'}</span>
                                        </li>
                                        <li className="flex items-center bg-neutral-100/80 hover:bg-neutral-200/90 border  cursor-pointer w-fit px-3 rounded-full gap-3 lowercase text-md text-neutral-700">
                                            <PiHeartBreakThin size={20}/>
                                            <span className="!capitalize">{moment(post?.lostDate).fromNow()|| 'N/A'}</span>
                                        </li>
                                    </ul>
                                    {!post?.isRecovered
                                        ? (
                                            <Dialog>                                                
                                                <DialogTrigger className="w-full" asChild>
                                                    <button className="px-5 py-2 bg-black hover:bg-black/90 w-full text-white mt-10 rounded-md">
                                                        {post?.postType === 'lost'
                                                            ? 'Found This!'
                                                            : 'This is Mine!'
                                                        }
                                                ...
                                                <DialogContent className="w-full md:w-9/12 lg:w-[700px]">
                                                    <DialogHeader>
                                                        ...
                                                        <DialogDescription>
                                                            Fill the all of details and give a valid given/received location 
                                                        </DialogDescription>
                                                    </DialogHeader>
                                                        <form onSubmit={handelRecoverItem}>
                                                            <div className="pt-5 flex justify-between gap-5">
                                                            ...
                                                            <div className="flex-grow-0 md:flex-grow w-full pt-5">
                                                                <label htmlFor="lostDate" className="mb-1 flex">Date</label>
                                                                <Popover>
                                                                    <PopoverTrigger className="w-full items-center" >
                                                                        <div className="flex w-full justify-between items-center text-sm px-4 rounded-md py-2 border">
                                                                            {moment(date).format('MM/DD/YYYY')}
                                                            ...
                                                            <div className="pt-5 flex justify-between gap-5 flex-wrap md:flex-nowrap overflow-x-hidden max-w-full">
                                                                <div className="flex-grow w-full">
                                                                    <label htmlFor="postype" className="mb-1 flex">User</label>
                                                            ...
                                                            <DialogFooter className={'!flex-col'}>
                                                                <div className="flex justify-between w-full items-center">
                                                                    <div>
                                                                        <input required id="term&condition" type="checkbox" />
                                                                        <label htmlFor="term&condition" className="ml-2">Accept all <strong>Terms</strong> and <strong>Conditions</strong></label>
                                                                    </div>
                                                                    <button className="px-5 my-5 flex gap-3 items-center py-2 bg-black text-white rounded-md hover:bg-black/90">
                                                                        Submit
                                                                        {isLoadingSaving && <CgSpinnerTwoAlt size={20} className="animate-spin" />}
                                                                    </button>
                                                                </div>
                                                                {isAnyError && (
                                                                    <div>
                                        ...
                                        : (
                                            <div className="border p-2 mt-10 rounded-xl flex gap-2">
                                                <div>
                                                    <CiCircleInfo className="text-sky-600" size={25}/>
                                                </div>
                                                <p>
                                                    This item is already recovered. Notable to recover
                                                </p>
                                            </div>
                                        )
                                    }
